...
// ── Test 6: URL format — /api/home_intercom/record?target=<room> ──
void test_upload_url_format_is_ha_api_endpoint(void) {
    HTTPUploader::upload(test_data, sizeof(test_data), "192.168.1.1", 8764, "\346\210\277\351\227\264", "");
...
void test_upload_adds_auth_header_with_token(void) {
    mock_http_set_response(200, R"({"ok":true})");
    HTTPUploader::upload(test_data, sizeof(test_data), "ha.local", 8123, "living", "test-token-123");
...
void test_upload_no_auth_header_without_token(void) {
    mock_http_set_response(200, R"({"ok":true})");
    HTTPUploader::upload(test_data, sizeof(test_data), "server", 8764, "study", "");
...
void test_upload_no_auth_header_with_null_token(void) {
    mock_http_set_response(200, R"({"ok":true})");
    HTTPUploader::upload(test_data, sizeof(test_data), "server", 8764, "study", nullptr);
    TEST_ASSERT_EQUAL_STRING("", _http_mock.last_auth_header.c_str());
}
...
int main(void) {
    ...
    RUN_TEST(test_upload_url_format_is_ha_api_endpoint);
