...
AudioRecorder::AudioRecorder()
    : _write_idx(0), _buffer(nullptr), _buffer_head(nullptr),
      _max_samples(0), _sample_rate(0), _total_bytes(0),
      _recording(false), _initialized(false),
      _timer_handle(nullptr), _overflow(false) {}

AudioRecorder::~AudioRecorder() {
    if (_recording) stop();
    if (_timer_handle) timerEnd((hw_timer_t *)_timer_handle);
    free(_buffer_head);
...
bool AudioRecorder::begin(uint32_t sample_rate, uint32_t max_secs) {
    ...
    Serial.printf("[%s] Buffer: %u samples (%u sec), PSRAM free: %u KB\n",
                  TAG, _max_samples, max_secs, info.total_free_bytes / 1024);
