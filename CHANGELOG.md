...
- **Trust-on-first-use hello (#32)** — after WiFi connects, `POST /api/home_intercom/devices/hello` registers the MAC. Recording waits until hello succeeds (orange LED). Audio `max_record_secs` from the payload is applied; per-button rooms stay local until #28.
