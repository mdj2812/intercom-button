...
- **Trust-on-first-use hello (#32)** — after WiFi connects, `POST /api/home_intercom/devices/hello` registers the MAC. Recording waits until hello succeeds (orange LED). While idle, hello repeats every 2 minutes to refresh HA last_seen (5-minute Online window). Revoked/pending heartbeats return to the orange wait; a network blip retries in 30s without dropping registration. Audio `max_record_secs` from the payload is applied; per-button rooms stay local until #28.
